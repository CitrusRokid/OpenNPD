/* Mex function for grid based NPD face detection; used to coarsely scan negative samples.
 *
 * rects = NPDGrid(model, I [, minFace, maxFace, numThreads])
 * 
 * Input:
 * 
 *   <model>: structure array of the learned detector.
 *   <I>: the original input image.
 *   [minFace]: minimal size of face to be detected
 *   [maxFace]: maximal size of face to be detected
 *   [numThreads]: the maximum number of computing threads. Positive double scalar. Default: the number of all available processors.
 *
 * Output:
 *
 ...
 *
 * Compile:
 *     Windows: mex -largeArrayDims -DWINDOWS 'COMPFLAGS=/openmp $COMPFLAGS' NPDGrid.cpp -v
 *     Linux:  mex -largeArrayDims -DLINUX 'CFLAGS=\$CFLAGS -fopenmp' 'LDFLAGS=\$LDFLAGS -fopenmp' NPDGrid.cpp -v
 * 
 * Reference:
...
{    
    ...
    {
        mexErrMsgTxt("Usage: rects = NPDGrid(model, I [, minFace, maxFace, numThreads])");
    }
    ...
    for(int k = 0; k < numScales; k++) // process each scale
    {
        ...
        int winStep = (int) floor(pWinSize[k] * 0.5);
